...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if let Ok(entries) = fs::read_dir("transitland-atlas/feeds") {
        ...
        for (key, feed) in bruhz.into_iter() {
            let pool = pool.clone();
            handles.push(threaded_rt.spawn(async move 
                 ...
                 */
                                        ...
                                        if gtfs.routes.len() > 0 as usize {
                                            let _ = client.query("INSERT INTO gtfs.static_feeds (onestop_feed_id,max_lat, max_lon, min_lat, min_lon) VALUES ($1, $2, $3, $4, $5)  on conflict do nothing;", &[
                                            &feed.id,
                                            &least_lat,
                                            &least_lon,
                                            &least_lat,
