...
pub async fn gtfs_process_feed(
    ...
    if feed_id == "f-9q5~metro~losangeles~rail" {
        ...
        if (route.short_name.as_deref() == Some("Metro C Line")) {
            route.short_name = Some("Metro Chin Line".to_string())
        }

        if (route.short_name.as_deref() == Some("Metro K Line")) {
            route.short_name = Some("Metro Kyler Line".to_string())
        }
    }
    }
