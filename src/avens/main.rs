...
async fn main() -> Result<()> {
    ...
    let args = Args::parse();
    println!(
        "Starting Avens (OSM Preprocessor) for region: {}",
        args.region
    );
    ...
    // Map: NodeID -> (Lat, Lon, Flags)
    let mut node_coords: ahash::AHashMap<i64, (f64, f64, u32)> = ahash::AHashMap::new();

    for obj in reader.iter() {
        if let Ok(osmpbfreader::OsmObj::Node(node)) = obj {
            let mut flags = 0;
            // Check for crossing tags
            // highway=crossing, railway=crossing, railway=level_crossing
            let is_crossing = node.tags.get("highway").map(|s| s.as_str()) == Some("crossing")
                || node.tags.get("railway").map(|s| s.as_str()) == Some("crossing")
                || node.tags.get("railway").map(|s| s.as_str()) == Some("level_crossing");

            if is_crossing {
                flags |= catenary::routing_common::osm_graph::node_flags::NODE_FLAG_CROSSING;
            }

            node_coords.insert(node.id.0, (node.lat(), node.lon(), flags));
        }
    }
    ...
    for obj in reader.iter() {
        match obj {
            Ok(osmpbfreader::OsmObj::Way(way)) => {
                ...
                if !should_include_way(&way.tags) {
                    continue;
                }
                ...
                for node_id in &way.nodes {
                    if let Some(&(lat, lon, _)) = node_coords.get(&node_id.0) {
                        let (tx, ty) =
                            slippy_map_tiles::lat_lon_to_tile(lat as f32, lon as f32, zoom_level);
                        min_x = min_x.min(tx);
                        max_x = max_x.max(tx);
                        min_y = min_y.min(ty);
                        max_y = max_y.max(ty);
    ...
    for ((_x, _y), buffer) in chunk_buffers.iter_mut() {
        ...
        for node_id in nodes_needed {
            if let Some(&(lat, lon, flags)) = node_coords.get(&node_id) {
                let mut tags = osmpbfreader::Tags::new();
                if flags & catenary::routing_common::osm_graph::node_flags::NODE_FLAG_CROSSING != 0
                {
                    tags.insert("catenary:crossing".into(), "yes".into());
                }
    ...
    for ((x, y), buffer) in chunk_buffers {
        ...
        if street_data.nodes.is_empty() && street_data.edges.is_empty() {
            // println!("Skipping empty chunk: chunk_{}_{}_{}.pbf", x, y, zoom_level);
            continue;
        }
...
fn convert_to_street_data(
    ...
    for (i, osm_node) in sorted_nodes.iter().enumerate() {
        ...
        let mut flags = 0;
        if osm_node.tags.contains_key("catenary:crossing") {
            flags |= catenary::routing_common::osm_graph::node_flags::NODE_FLAG_CROSSING;
        }
    ...
    for way in buffer.ways {
        for i in 0..way.nodes.len() - 1 {
                ...
                (osm_id_to_idx.get(&u_id), osm_id_to_idx.get(&v_id))
            {
                ...
                // Parse Tags for Flags
                // Road Class
                if let Some(highway) = way.tags.get("highway").map(|s| s.as_str()) {
                    let class_flag = match highway {
                        "motorway" | "motorway_link" => edge_flags::CLASS_MOTORWAY,
                        "trunk" | "trunk_link" => edge_flags::CLASS_TRUNK,
                        "primary" | "primary_link" => edge_flags::CLASS_PRIMARY,
                        "secondary" | "secondary_link" => edge_flags::CLASS_SECONDARY,
                        "tertiary" | "tertiary_link" => edge_flags::CLASS_TERTIARY,
                        "residential" | "unclassified" | "service" | "living_street" => {
                            edge_flags::CLASS_RESIDENTIAL
                        }
                        // Everything else (footway, cycleway, path, track, steps, pedestrian) -> CLASS_PATH (0)
                        _ => edge_flags::CLASS_PATH,
                    };
                    flags |= class_flag;
                }

                // Sidewalk
                // Check "sidewalk" tag or if it's a footway/path
                let has_sidewalk = way.tags.get("sidewalk").map(|s| s.as_str()).unwrap_or("no")
                    != "no"
                    || way.tags.get("highway").map(|s| s.as_str()) == Some("footway")
                    || way.tags.get("highway").map(|s| s.as_str()) == Some("path")
                    || way.tags.get("highway").map(|s| s.as_str()) == Some("pedestrian")
                    || way.tags.get("highway").map(|s| s.as_str()) == Some("steps");

                if has_sidewalk {
                    flags |= edge_flags::EDGE_FLAG_HAS_SIDEWALK;
                }

                // Edge Crossing (footway=crossing, cycleway=crossing, etc.)
                let is_edge_crossing = way.tags.get("footway").map(|s| s.as_str())
                    == Some("crossing")
                    || way.tags.get("cycleway").map(|s| s.as_str()) == Some("crossing")
                    || way.tags.get("path").map(|s| s.as_str()) == Some("crossing")
                    || way.tags.get("bridleway").map(|s| s.as_str()) == Some("crossing");

                if is_edge_crossing {
                    flags |= edge_flags::EDGE_FLAG_CROSSING;
                }
...
fn parse_kml_boundary(content: &str) -> Result<geo::MultiPolygon<f64>> {
    ...
    // Find all occurrences of <coordinates>...</coordinates>
    let mut start_search_idx = 0;

    while let Some(start_tag_idx) = content[start_search_idx..].find("<coordinates>") {
        let absolute_start = start_search_idx + start_tag_idx;
        let content_after_start = &content[absolute_start..];

        if let Some(end_tag_idx) = content_after_start.find("</coordinates>") {
            let coords_str = &content_after_start[13..end_tag_idx];

            let mut exterior = Vec::new();
            // Split by whitespace (newlines, spaces)
            for coord_pair in coords_str.split_whitespace() {
            ...
            start_search_idx = absolute_start + end_tag_idx + 14;
        } else {
            break;
        }
    }
...
fn calculate_covering_tiles(boundary: &geo::MultiPolygon<f64>, zoom: u8) -> Vec<(u32, u32)> {
    ...
    let (min_x, min_y) =
        slippy_map_tiles::lat_lon_to_tile(bbox.max().y as f32, bbox.min().x as f32, zoom); // Top-Left (Lat is Max, Lon is Min)
    let (max_x, max_y) =
        slippy_map_tiles::lat_lon_to_tile(bbox.min().y as f32, bbox.max().x as f32, zoom); // Bottom-Right (Lat is Min, Lon is Max)

    let mut tiles = Vec::new();

    // Note: Tile Y coordinates go from North to South (0 at Top), so min_lat corresponds to max_y!
    // We need to handle ranges correctly.
...
fn should_include_way(tags: &osmpbfreader::Tags) -> bool {
    match tags.get("highway").map(|s| s.as_str()) {
        Some("motorway") | Some("motorway_link") => {
            ...
            // Check for cycleway
            if let Some(cycleway) = tags.get("cycleway") {
                if cycleway != "no" && cycleway != "none" {
                    return true;
                }
            }
...
mod tests {
    ...
    fn test_should_include_way() {
        ...
        // Motorway without sidewalk/cycleway
        tags.insert("highway".into(), "motorway".into());
        assert!(!should_include_way(&tags));

        // Motorway with sidewalk=no
        tags.insert("sidewalk".into(), "no".into());
        assert!(!should_include_way(&tags));

        // Motorway with sidewalk=both
        tags.insert("sidewalk".into(), "both".into());
        assert!(should_include_way(&tags));
        ...
        // Motorway with cycleway=lane
        tags.insert("cycleway".into(), "lane".into());
        assert!(should_include_way(&tags));

        // Motorway link
        tags.insert("highway".into(), "motorway_link".into());
