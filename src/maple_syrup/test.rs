fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    .get::<String>("path").expect("expected path to gtfs file like --path FILE.zip");
    ...
    let stop_times_count = gtfs.trips.iter().map(|(_, trip)| trip.stop_times.len()).sum::<usize>();

    let iten_count = response.itineraries.len();

    println!("Reduced schedule in {:?}", duration);
    println!("{} has {} routes, {} trips and {} stop times, reduced to {} itineraries", path, route_count, trip_count, stop_times_count, iten_count);

    println!("Compression ratio: {:.2}", (trip_count) as f64 / iten_count as f64);
