...
pub async fn run_sncb_importer(sncb_data: Arc<SncbSharedData>, pool: Arc<CatenaryPostgresPool>) {
    println!("[SNCB] Starting SNCB Importer...");

    let clients = vec![Arc::new(ProxyClient {
...

        {
            ...
            if let Ok(mut conn) = pool.get().await {
                ...
                if let Ok(trips) = trips_result {
                    ...
                    println!(
                        "[SNCB] Fetched {} active SNCB trips to query",
                        trip_ids_to_fetch.len()
                    );
                } else {
        ...
        for short_name in trip_ids_to_fetch.iter() {
            ...
            tokio::spawn(async move {
                match client_clone.client.get(&url).send().await {
                    Ok(resp) => {
                        if resp.status() == 429 {
                            ...
                            if let Ok(data) = resp.json::<IRailVehicleResponse>().await {
                                ...
                                sncb_data_clone
                                    .upsert_async(data.vehicleinfo.shortname.clone(), data)
                                    .await;
                            }
                        } else {
                            // 404 or other error
                            println!(
                                "[SNCB] Failed to fetch {} ({}): {}",
                                short_name_clone,
                                client_clone.name,
                                resp.status()
                            );
                    ...
                    Err(e) => {
                        println!(
                            "[SNCB] Request failed for {} ({}): {}",
                            short_name_clone, client_clone.name, e
                        );
                    }
                }
            });
