...
pub async fn new_rt_data(
   ...
   let vehicle = match vehicles {
        Some(v) => {
            match parse_gtfs_rt_message(&v.as_slice()) {
                Ok(v) => Some(v),
                Err(e) => {
    ...
    let trip = match trips {
        Some(t) => {
            match parse_gtfs_rt_message(&t.as_slice()) {
                Ok(t) => Some(t),
                Err(e) => {
    ...
    let alert = match alerts {
        Some(a) => {
            match parse_gtfs_rt_message(&a.as_slice()) {
                Ok(a) => Some(a),
                Err(e) => {
    ...
    let mutable_raw_gtfs_rt = this_chateau_lock.raw_gtfs_rt.get_mut(&realtime_feed_id.clone());
    ...
    match mutable_raw_gtfs_rt {
        ...
        None => {
            ...
            this_chateau_lock.raw_gtfs_rt.insert(realtime_feed_id.clone(), new_gtfs_rt_data);
        }
    }
    ...
    let mut trip_updates_lookup_by_trip_id_to_trip_update_ids:HashMap<String, Vec<String>> = HashMap::new();
