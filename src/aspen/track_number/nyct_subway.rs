    ...
    for (url, res) in NYCT_FEED_URLS.iter().zip(results) {
        if let Ok(response) = res {
            if response.status().is_success() {
                                ...
                                for stu in trip_update.stop_time_update {
                                    if let Some(nyct_ext) = stu.nyct_stop_time_update {
                                        if let Some(stop_id) = &stu.stop_id {
                                            let has_actual = nyct_ext.actual_track.is_some() && !nyct_ext.actual_track.as_ref().unwrap().is_empty();
                                            let has_sched = nyct_ext.scheduled_track.is_some() && !nyct_ext.scheduled_track.as_ref().unwrap().is_empty();
                                            
                                            if has_actual || has_sched {
                                                let trip_entry = track_lookup.entry(trip_id.clone()).or_default();
                                                let track_pair = trip_entry.entry(stop_id.clone()).or_default();
                                                
                                                if has_actual {
                                                    track_pair.actual_track = nyct_ext.actual_track.clone();
                                                }
                                                if has_sched {
                                                    track_pair.scheduled_track = nyct_ext.scheduled_track.clone();
                                                }
                                            }
                                        }
                                    }
                ...
                eprintln!("Failed to fetch NYCT GTFS-RT feed: {} (Status: {})", url, response.status());
            }
        } else {
