    ...
    for ((chateau, key), meta) in station_group_metadata {
        if let Some(mut deps) = ld_departures_by_group.remove(&(chateau.clone(), key.clone())) {
            ...
            let rep_id_fixed = if rep_id.is_empty() {
                station_groups
                    .get(&(chateau.clone(), key.clone()))
                    .and_then(|v| v.as_slice().first())
                    .map(|s| s.gtfs_id.clone())
                    .unwrap_or_default()
            } else {
                rep_id
            };
...
async fn fetch_chateau_data(
    ...
    let calendar_struct =
        make_calendar_structure_from_pg(vec![calendar.clone()], vec![calendar_dates.clone()])
            .unwrap_or_default();
                    ...
                    Ok(resp) => {
                        if let Some(kv) = resp.kvs().first() {
                            match catenary::bincode_deserialize::<ChateauMetadataEtcd>(kv.value()) {
                                Ok(meta) => {
                                    // Connect Aspen
                                    match catenary::aspen::lib::spawn_aspen_client_from_ip(
                                        &meta.socket,
                                    )
                                    .await
                                    {
                                        Ok(client) => {
                                            let t_fut = client.get_all_trips_with_ids(
                                                tarpc::context::current(),
                                                chateau.clone(),
                                                trip_ids.clone(),
                                            );
                                            let a_fut = client.get_all_alerts(
                                                tarpc::context::current(),
                                                chateau.clone(),
                                            );
                                            ...
                                            match t {
                                                ...
                                                Ok(None) => {
                                                    println!(
                                                        "RT_DEBUG [{}]: Aspen returned None for trips",
                                                        chateau
                                                    );
                                                }
                                                Err(e) => {
                                                    eprintln!(
                                                        "RT_DEBUG [{}]: Failed to get trips from aspen: {:?}",
                                                        chateau, e
                                                    );
                                                }
                                            }
                                            match a {
                                                ...
                                                Err(e) => {
                                                    eprintln!(
                                                        "RT_DEBUG [{}]: Failed to get alerts from aspen: {:?}",
                                                        chateau, e
                                                    );
                                                }
                                            }
                                        ...
                                        Err(e) => {
                                            eprintln!(
                                                "RT_DEBUG [{}]: Failed to connect to aspen at {:?}: {:?}",
                                                chateau, meta.socket, e
                                            );
                                        }
                                    }
                                }
                                Err(e) => {
                                    eprintln!(
                                        "RT_DEBUG [{}]: Failed to deserialize chateau metadata: {:?}",
                                        chateau, e
                                    );
                                }
                            }
        ...
        for row in rows {
                ...
                stop_to_key_map.get(&(chateau.clone(), row.stop_id.to_string()))
            {
                ...
                for date in &valid_service_dates {
                    ...
                    let tz = chrono_tz::Tz::from_str_insensitive(&timezone_str)
                    ...
                    for alert in relevant_alerts {
                        ...
                        if alert.effect == Some(1) {
                            ...
                            let is_active = alert.active_period.iter().any(|ap| {
                                ...
                                (event_time as u64) >= start && (event_time as u64) <= end
                            });
                            if is_active {
                                is_cancelled = true;
                            }
                                ...
                                if let Some(u) = data.trip_updates.get(uid) {
                                    ...
                                    if u.trip.start_date.is_none() {
                                        let valid_trip_temp = ValidTripSet {
                                            ...
                                            reference_start_of_service_date: tz
                                                .from_local_datetime(
                                        ...
                                        let itin_option_temp = ItinOption {
                                            ...
                                            departure_time_since_start: row
                                                .departure_time_since_start,
                                            interpolated_time_since_start: row
