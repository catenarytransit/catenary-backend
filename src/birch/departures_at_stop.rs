...
pub async fn departures_at_stop(req: HttpRequest, query: Query<NearbyFromStops>,
    ...
    let itins_ids = itins.iter().map(|x| x.itinerary_pattern_id.clone()).collect::<Vec<String>>();
    ...
    catenary::schema::gtfs::itinerary_pattern_meta::dsl::itinerary_pattern_meta
        .filter(catenary::schema::gtfs::itinerary_pattern_meta::chateau.eq(query.chateau_id.clone()))
        .filter(catenary::schema::gtfs::itinerary_pattern_meta::itinerary_pattern_id.eq_any(itins_ids.clone()))
    ...
    .filter(catenary::schema::gtfs::trips_compressed::itinerary_pattern_id.eq_any(itins_ids.clone()))
